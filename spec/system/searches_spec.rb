  ...
  describe "search articles" do
    ...
    context "fill in search word" do
      ...
      before do 
        ...
        @article2 = user.articles.create(title: "サケ類の母川回帰性の種間比較", content: "日本に生息するサケ", category_id: fishery.id, tag_list: nil)
        @article3 = user.articles.create(title: "漁と文化の歴史", content: "本研究は水産学的な観点から議論する。", category_id: environmentology.id, tag_list: "日本")
        @article4 = user.articles.create(title: "日本の海洋ゴミ問題", content: "海洋ゴミが増加している", category_id: nil, tag_list: "水産学")
        @article5 = user.articles.create(title: "魚類生態学", content: "いろいろな水産物", category_id: zoology.id, tag_list: nil)
        @article6 = user.articles.create(title: "魚類生理学", content: "物質が働く", category_id: zoology.id, tag_list: nil)
        
        @article7 = user.articles.create(title: "日本の医療技術", content: "最新の知見", category_id: medicine.id, tag_list: nil)
        @article8 = user.articles.create(title: "カタクチイワシの漁獲量変動", content: "水産学的に重要", category_id: fishery.id, tag_list: nil)
      end
